leaderboard = {}
while True:
    print("\n 1.Add Score\n2.Show LeaderBoard 3. Check Rank \n4.Exit")
    ...
    if choice == "1":
        ...
        for name, data in sorted_board:
            if name==player_name:
                print(f"{player}is ranked {rank} with {data["score"]} points")
                found = True
                break
            rank +=1
            if not found:
                print(f"{player_name} is not on the leadership board yet")
    elif choice =="4":
        print("Goodby")
        break

else:
    print("Invalid choice! Choose a numver 1-4")
